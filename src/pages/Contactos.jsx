/**
 * Contactos.jsx — Tu versión original completa.
 * Único añadido: botones 📞 y 📹 en el PanelDerecho (desktop)
 * que usan iniciarLlamada() del LlamadaContext.
 */
...
export default function Contactos() {
  ...
  const { iniciarLlamada } = useLlamada()   // ← único añadido
  ...
  // ── Llamar desde panel derecho (desktop) ──────────────────
  const llamarContacto = (c, tipo) => {
    const chatId = [user.email, c.contactoEmail].sort().join('_')
    iniciarLlamada({ chatId, tipo, contacto: c })
  }
  ...
  const Sidebar = () => (
    <div style={{
      ...
      height:'100%', overflow:'hidden', transition:'background 0.3s'
    }}>
      {/* Header */}
      ...
      <div style={{ padding:'10px 12px', borderBottom:`1px solid ${C.sideBorder}`, flexShrink:0 }}>
        <input type="text" value={searchSide} onChange={e=>setSearchSide(e.target.value)}
        ...
        {contactosFiltrados.map(c => {
          ...
          return (
            <div key={c.id}
              ...
              style={{ display:'flex', alignItems:'center', gap:'12px', padding:'12px 16px', cursor:'pointer', borderBottom:`1px solid ${C.divider}`, background:isActive?C.rowActive:'transparent', transition:'background 0.15s' }}
      ...
      <div style={{ padding:'10px 12px', borderTop:`1px solid ${C.sideBorder}`, flexShrink:0 }}>
        ...
        <div style={{ display:'flex', gap:'6px' }}>
          <input value={busqueda} onChange={e=>{setBusqueda(e.target.value);setError('');setMsg('')}}
      ...
      {!selContact ? (
        ...
        <div style={{ flex:1, display:'flex', flexDirection:'column', alignItems:'center', padding:'48px 32px 32px', gap:'16px', overflowY:'auto' }}>
          ...
          <div style={{ display:'flex', flexDirection:'column', gap:'10px', width:'100%', maxWidth:'280px', marginTop:'8px' }}>
            ...
            {/* ── Botones de llamada (añadido) ── */}
            <div style={{ display:'flex', gap:'10px' }}>
              <button onClick={() => llamarContacto(selContact, 'audio')} style={{ flex:1, background:dark?'rgba(15,118,110,0.25)':'#e6faf5', color:'#0f766e', border:'1px solid rgba(15,118,110,0.3)', borderRadius:'12px', padding:'12px', fontSize:'14px', fontWeight:'600', cursor:'pointer', display:'flex', alignItems:'center', justifyContent:'center', gap:'6px' }}>
                📞 Llamar
              </button>
              <button onClick={() => llamarContacto(selContact, 'video')} style={{ flex:1, background:dark?'rgba(8,145,178,0.25)':'#e0f7fa', color:'#0891b2', border:'1px solid rgba(8,145,178,0.3)', borderRadius:'12px', padding:'12px', fontSize:'14px', fontWeight:'600', cursor:'pointer', display:'flex', alignItems:'center', justifyContent:'center', gap:'6px' }}>
                📹 Video
              </button>
            </div>
